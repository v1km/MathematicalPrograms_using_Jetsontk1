//PROGRAM6
//THIS PROGRAM DEMONSTRATE VECTOR-MATRIX MULTIPLICATION USING 1D TEXTURE MEMORY
#include<stdio.h>
#include<cuda.h>
...
/***************************************************************/
 ...
 /****************************************************************/  
  ...
  //Copy V and M  from CPU to GPU
	...
	//Bind the texture
	 if(cudaSuccess!=cudaBindTexture(NULL, gVTex, gV, Vsize*sizeof(float)))
     {
            printf("\n Texture binding failed for  gVTex");
           exit(EXIT_FAILURE);
    }
...
/************************************************************************/
  //Copy Result back to CPU in R
    ...
    /********************************************************************/
     //Unbind the texture
     cudaUnbindTexture(gVTex);
    /*******************************************************************/
     //DO CLEAN UP  
	  free(V); free(M); free(R); //Host pointers
      cudaFree(gV); cudaFree(gM); cudaFree(gR); //Device pointer
	/********************************************************************/
   getchar();
   return 0;
}
...
__global__ void VMMulti( float*gM, float *gR, int Vsize, int Mcols)
{
	...
	if(i<Mcols)
	{
	  ...
	  for(j=0;j<Vsize;j++)
	  {
		  x=tex1Dfetch(gVTex,j);
		  sum=sum+(x*gM[j+(i*Vsize)]);
	  }
	     __syncthreads();
	    gR[i]=sum;
	}
