//PROGRAM10
//THIS IS A MULTI CUDA BLOCK REDUCTION EXAMPLE
//IT FIND OUT MINIMUM AMONG ALL THE VALUES USING A CUDA KERNEL
#include<stdio.h>
#include<cuda.h>
__global__ void ReductionKernel(int, int *,int* ,int);
...
int main()
{
	...
	int size=5000, blocksize=256, blocks;
	int flag;// used to exit from the kernel looping
	int count;//used to count kernel calls
	int values;// these values need to be reduced per kernel call 
  ...
  /*******************************************************************/
	  //Allocate space for input array pointed by input on GPU        
	   //check memory allocation on GPU
		 ...
		 if (cudaSuccess!=cudaMalloc((void **)&d_output,size*sizeof(int)))
	     {
		...
		values=size;
		count=0;
		//Do the kernel looping using while loop
			do
			{
			 //Compute No of blocks needed 
			  blocks=(int)(values/blocksize);			     
			  if((values%blocksize)>0)
			  blocks++;
			 //set the flag if only one block need to be reduced
			  if(values<blocksize)
				flag=1;
			  //call the kernel to reduce the values
			  ReductionKernel<<<blocks,blocksize,sh>>>(count,d_input,d_output, values); 
			  cudaDeviceSynchronize();	
			  //increase the count so that next time kernel will read the values from d_Min				   
					  count++;
			  //check for any error in CUDA kernel	
			  //set the new number of values to be reduced
					  values=blocks; 				
			}while(flag!=1);	   
...
__global__ void ReductionKernel(int count,int *d_input, int* d_output,int values)
{
	int i=threadIdx.x;
    ...
    //copy max in s[i]
      ...
      //copy the values from d_input in s[i] 
        if (i<values)
		{
             if(count==0)//reduce from d_input
			     s[i]=d_input[index];	
			 else  //reduce from d_Min
				 s[i]=d_output[index];	
